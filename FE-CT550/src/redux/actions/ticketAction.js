...
export const createTicket = (user_id, tripPassengerId) => {
	return async (dispatch) => {
		try {
			const result = await ticketService.createTicket(user_id, tripPassengerId);
			if (result.status == 200) {
				message.success("Thêm vé xe khách thành công");
				dispatch(getAllTicket());
			}
		} catch (error) {
			message.error("Thêm vé xe khách thất bại");
			console.log(error);
		}
	};
};
